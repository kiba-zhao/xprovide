...
const { isClass, isArray, isString, isFunction, isPlainObject, pkg, defaults, isBoolean } = require('./utils');
...
class Provider {
  ...
  require(deps, success, fatal) {
    assert(isArray(deps) && deps.every(isDep), `[${pkg.name}] Provider:require: wrong deps argument`);
  ...
  define(id, deps, factory, opts = {}) {
    ...
    assert(isArray(deps) && deps.every(isDep), `[${pkg.name}] Provider:define: wrong deps argument`);
...
function buildModel(provider, deps, action) {
  ...
  for (const dep of deps) {
    ...
    if (item) {
      ...
      assert(required === false, `[${pkg.name}] Provider:dep ${id} is required`);
    }
